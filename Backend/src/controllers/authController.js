...
const register = async (req, res, next) => {
    try {
        ...
        console.log("[AUTH] Connecting to DB to check for existing user...");
        const { rows: existingRows } = await db.query('SELECT * FROM "User" WHERE email = $1', [email]);
        console.log("[AUTH] DB Result (Existing User):", existingRows.length);
        ...
        console.log("[AUTH] Inserting new user into DB...");
        const { rows: insertRows } = await db.query(
            'INSERT INTO "User" (name, phone, email, password, "isVerified") VALUES ($1, $2, $3, $4, true) RETURNING id',
            [name, phone, email, hashedPassword]
        );
        console.log("[AUTH] User inserted successfully ID:", insertRows[0].id);
        ...
        console.error("[AUTH] CRITICAL ERROR during Register:", e.message);
        next(e);
    }
};
